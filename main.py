...
# ── InlineKeyboardButton compatibility patch ───────────────────────
# On older aiogram (< 3.13) the `style` field does not exist on
# InlineKeyboardButton.  Depending on the pydantic version aiogram
# uses, the `extra` config can be `"ignore"` (kwarg silently dropped)
# or `"forbid"` (ValidationError on unknown kwarg).  Detect the field
# at import time via pydantic's actual field registry, and patch the
# constructor to drop the `style` kwarg when it isn't supported.
...
if not _IKB_HAS_STYLE:
    ...
    def _patched_ikb_init(self, *args, **kwargs):
        # Silently drop the `style` kwarg — the button will still
        # work, it just won't be coloured in older aiogram.
    ...
    print("INFO: Patched InlineKeyboardButton to drop 'style' kwarg "
          "(aiogram < 3.13). Colours will not render until upgrade.")
