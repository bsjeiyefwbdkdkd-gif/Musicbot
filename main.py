...
═══════════════════════════════════════════════════════════════════════
  ...
  Python 3.12  •  aiogram 3.30  •  SQLite (async via aiosqlite)
  Polling mode  •  Railway-ready  •  GitHub-deployable  •  Production-grade
═══════════════════════════════════════════════════════════════════════
...
# ── ButtonStyle — native aiogram 3.30.0 import ──────────────────────
# In aiogram 3.30.0 the `ButtonStyle` enum lives at
# `aiogram.enums.button_style`. The `style` field on
# `InlineKeyboardButton` is supported natively, so no monkey-patching
# is required. We keep a graceful fallback for older versions.
#
# Telegram's Bot API only accepts three colour values:
#     PRIMARY  = "primary"  (blue)
#     SUCCESS  = "success"  (green)
#     DANGER   = "danger"   (red)
# Anything else (or omitted) falls back to the app's default style.
try:
    from aiogram.enums.button_style import ButtonStyle  # aiogram 3.30+
    ...
    try:
        ...
        class ButtonStyle(str, Enum):
            """Local fallback — only the 3 values Telegram accepts."""
            PRIMARY = "primary"
            SUCCESS = "success"
            DANGER = "danger"
        _BUTTON_STYLE_NATIVE = False

# Verify the `style` field is actually present on InlineKeyboardButton
# (aiogram >= 3.26 exposes it). If not, we strip the kwarg so the
# constructor doesn't reject it.
if hasattr(InlineKeyboardButton, "model_fields"):
    _IKB_HAS_STYLE = "style" in InlineKeyboardButton.model_fields
else:
    _IKB_HAS_STYLE = "style" in getattr(InlineKeyboardButton,
                                         "__fields__", {})

if not _IKB_HAS_STYLE:
    _ORIG_IKB_INIT = InlineKeyboardButton.__init__

    def _patched_ikb_init(self, *args, **kwargs):
        # Older aiogram — silently drop `style` so the button still
        # builds, even though it won't render in colour.
        kwargs.pop("style", None)
        _ORIG_IKB_INIT(self, *args, **kwargs)

    InlineKeyboardButton.__init__ = _patched_ikb_init
    print("INFO: Patched InlineKeyboardButton to drop 'style' kwarg "
          "(aiogram < 3.26). Colours will not render until upgrade.")

# ── HTML escape helper for user-controlled strings ──────────────────
import html as _html


def _esc(value) -> str:
    """Escape user-controlled text for safe inclusion in HTML messages."""
    if value is None:
        return ""
...
# ── Database path ────────────────────────────────────────────────────
# On Railway the container's filesystem is ephemeral, so the SQLite
# DB would be wiped on every redeploy unless we point it at a mounted
# volume. By default we try /data (the conventional Railway volume
# mount point) and fall back to the working directory. Both DATA_DIR
# and DB_FILE env vars are overridable.
...
if _BUTTON_STYLE_NATIVE and _IKB_HAS_STYLE:
    print("  ButtonStyle: native (coloured buttons will render ✅)")
else:
    print("  ButtonStyle: FALLBACK — coloured buttons require"
          " aiogram>=3.26")
    print("  Buttons will work, but the visual style will not be sent"
          " to Telegram.")
    print("  -> Upgrade with:  pip install --upgrade 'aiogram>=3.30'")
print("=" * 60)
...
async def _send_song(bot: Bot, chat_id: int, song, user_id: int) -> None:
    ...
    try:
        ...
        logger.error("send_song error for id=%s: %s", song["id"] if song else "?", e, exc_info=True)
        try:
...
async def cmd_start(message: Message, bot: Bot, state: FSMContext):
    try:
        ...
        await message.answer(
            ...
            f"{_esc(user.first_name) or 'دوست عزیز'}!</b>\n\n"
        ...
        try:
            await message.answer(
                ...
                f"<i>Error: {_esc(type(e).__name__)}: {_esc(str(e))[:200]}</i>",
                parse_mode=ParseMode.HTML,
            )
        except Exception:
            try:
    ...
    try:
        ...
        if await check_membership(bot, user.id):
            await call.message.edit_text(
                f"🎵 <b>خوش آمدید، {_esc(user.first_name) or 'دوست عزیز'}!</b>\n\n"
                "✨ از منوی زیر گزینه مورد نظر را انتخاب کنید:",
                reply_markup=main_menu_keyboard(is_admin(user.id)).as_markup(),
                parse_mode=ParseMode.HTML,
            )
        ...
        logger.error("check_membership error: %s", e, exc_info=True)
        try:
...
async def cb_back_home(call: CallbackQuery, state: FSMContext):
    try:
        ...
        logger.error("back_home error: %s", e, exc_info=True)
        try:
            await call.answer("❌ خطایی رخ داد.", show_alert=True)
        except Exception:
            pass
...
async def cb_about(call: CallbackQuery):
    try:
        ...
        logger.error("about error: %s", e, exc_info=True)
        await call.answer("❌ خطایی رخ داد.", show_alert=True)
...
async def cb_search(call: CallbackQuery):
    try:
        ...
        logger.error("search menu error: %s", e, exc_info=True)
        await call.answer("❌ خطایی رخ داد.", show_alert=True)
...
async def cb_search_by_name(call: CallbackQuery, state: FSMContext):
    try:
        ...
        logger.error("search_by_name error: %s", e, exc_info=True)
        await call.answer("❌ خطایی رخ داد.", show_alert=True)
...
async def cb_search_by_artist(call: CallbackQuery, state: FSMContext):
    try:
        ...
        logger.error("search_by_artist error: %s", e, exc_info=True)
        await call.answer("❌ خطایی رخ داد.", show_alert=True)
...
async def cb_search_by_category(call: CallbackQuery):
    try:
        ...
        logger.error("search_by_category error: %s", e, exc_info=True)
        await call.answer("❌ خطایی رخ داد.", show_alert=True)
...
async def cb_scat(call: CallbackQuery):
    try:
        ...
        logger.error("scat error: %s", e, exc_info=True)
        await call.answer("❌ خطایی رخ داد.", show_alert=True)
...
async def msg_search_query(message: Message, state: FSMContext):
    try:
        ...
        logger.error("search_query error: %s", e, exc_info=True)
        await state.clear()
        try:
...
async def cb_upload(call: CallbackQuery, state: FSMContext):
    try:
        ...
        logger.error("upload start error: %s", e, exc_info=True)
        await call.answer("❌ خطایی رخ داد.", show_alert=True)
...
async def msg_upload_name(message: Message, state: FSMContext):
    try:
        ...
        logger.error("upload name error: %s", e, exc_info=True)
        await state.clear()
...
async def msg_upload_artist(message: Message, state: FSMContext):
    try:
        ...
        logger.error("upload artist error: %s", e, exc_info=True)
        await state.clear()
...
async def cb_upload_cat(call: CallbackQuery, state: FSMContext):
    try:
        ...
        logger.error("upload cat error: %s", e, exc_info=True)
        await call.answer("❌ خطایی رخ داد.", show_alert=True)
...
async def cb_skip_cover(call: CallbackQuery, state: FSMContext):
    try:
        ...
        logger.error("skip_cover error: %s", e, exc_info=True)
        await call.answer("❌ خطایی رخ داد.", show_alert=True)
...
async def msg_upload_cover(message: Message, state: FSMContext):
    try:
        ...
        logger.error("upload cover error: %s", e, exc_info=True)
        await state.clear()
...
async def msg_upload_audio(message: Message, state: FSMContext):
    try:
        ...
        logger.error("upload audio error: %s", e, exc_info=True)
        try:
...
async def cb_favorites(call: CallbackQuery):
    try:
        ...
        logger.error("favorites error: %s", e, exc_info=True)
        await call.answer("❌ خطایی رخ داد.", show_alert=True)
...
async def cb_add_fav(call: CallbackQuery):
    try:
        ...
        logger.error("add_fav error: %s", e, exc_info=True)
        await call.answer("❌ خطایی رخ داد.", show_alert=True)
...
async def cb_remove_fav(call: CallbackQuery):
    try:
        ...
        logger.error("remove_fav error: %s", e, exc_info=True)
        await call.answer("❌ خطایی رخ داد.", show_alert=True)
...
async def cb_top_downloads(call: CallbackQuery):
    try:
        ...
        logger.error("top_downloads error: %s", e, exc_info=True)
        await call.answer("❌ خطایی رخ داد.", show_alert=True)
...
async def cb_my_uploads(call: CallbackQuery):
    try:
        ...
        logger.error("my_uploads error: %s", e, exc_info=True)
        await call.answer("❌ خطایی رخ داد.", show_alert=True)
...
async def cb_myu_view(call: CallbackQuery):
    try:
        ...
        logger.error("myu_view error: %s", e, exc_info=True)
        await call.answer("❌ خطایی رخ داد.", show_alert=True)
...
async def cb_myu_del(call: CallbackQuery):
    try:
        ...
        logger.error("myu_del error: %s", e, exc_info=True)
        await call.answer("❌ خطایی رخ داد.", show_alert=True)
...
async def cb_random(call: CallbackQuery):
    try:
        ...
        logger.error("random error: %s", e, exc_info=True)
        await call.answer("❌ خطایی رخ داد.", show_alert=True)
...
async def cb_categories(call: CallbackQuery):
    try:
        ...
        logger.error("categories menu error: %s", e, exc_info=True)
        await call.answer("❌ خطایی رخ داد.", show_alert=True)
...
async def cb_cat_view(call: CallbackQuery):
    try:
        ...
        logger.error("cat_view error: %s", e, exc_info=True)
        await call.answer("❌ خطایی رخ داد.", show_alert=True)
...
async def cb_admin_panel(call: CallbackQuery):
    try:
        ...
        logger.error("admin_panel error: %s", e, exc_info=True)
        await call.answer("❌ خطایی رخ داد.", show_alert=True)
...
async def cb_admin_stats(call: CallbackQuery):
    try:
        ...
        logger.error("admin_stats error: %s", e, exc_info=True)
        await call.answer("❌ خطایی رخ داد.", show_alert=True)
...
async def cb_admin_users(call: CallbackQuery):
    try:
        ...
        logger.error("admin_users error: %s", e, exc_info=True)
        await call.answer("❌ خطایی رخ داد.", show_alert=True)
...
async def cb_admin_songs(call: CallbackQuery):
    try:
        ...
        logger.error("admin_songs error: %s", e, exc_info=True)
        await call.answer("❌ خطایی رخ داد.", show_alert=True)
...
async def cb_admin_delete_song(call: CallbackQuery, state: FSMContext):
    try:
        ...
        logger.error("admin_delete_song error: %s", e, exc_info=True)
        await call.answer("❌ خطایی رخ داد.", show_alert=True)
...
async def msg_admin_delete_song(message: Message, state: FSMContext):
    try:
        ...
        logger.error("admin_delete_song finish error: %s", e, exc_info=True)
        await state.clear()
...
async def cb_admin_broadcast(call: CallbackQuery, state: FSMContext):
    try:
        ...
        logger.error("admin_broadcast start error: %s", e, exc_info=True)
        await call.answer("❌ خطایی رخ داد.", show_alert=True)
...
async def msg_admin_broadcast(message: Message, state: FSMContext, bot: Bot):
    try:
        ...
        logger.error("admin_broadcast error: %s", e, exc_info=True)
        try:
...
async def cb_admin_ban(call: CallbackQuery, state: FSMContext):
    try:
        ...
        logger.error("admin_ban error: %s", e, exc_info=True)
        await call.answer("❌ خطایی رخ داد.", show_alert=True)
...
async def msg_admin_ban(message: Message, state: FSMContext):
    try:
        ...
        logger.error("admin_ban finish error: %s", e, exc_info=True)
        await state.clear()
...
async def cb_admin_unban(call: CallbackQuery, state: FSMContext):
    try:
        ...
        logger.error("admin_unban error: %s", e, exc_info=True)
        await call.answer("❌ خطایی رخ داد.", show_alert=True)
...
async def msg_admin_unban(message: Message, state: FSMContext):
    try:
        ...
        logger.error("admin_unban finish error: %s", e, exc_info=True)
        await state.clear()
...
async def cb_admin_add_category(call: CallbackQuery, state: FSMContext):
    try:
        ...
        logger.error("admin_add_category error: %s", e, exc_info=True)
        await call.answer("❌ خطایی رخ داد.", show_alert=True)
...
async def msg_admin_add_category(message: Message, state: FSMContext):
    try:
        ...
        logger.error("admin_add_category finish error: %s", e, exc_info=True)
        await state.clear()
...
async def cb_admin_delete_category(call: CallbackQuery):
    try:
        ...
        logger.error("admin_delete_category error: %s", e, exc_info=True)
        await call.answer("❌ خطایی رخ داد.", show_alert=True)
...
async def cb_del_cat(call: CallbackQuery):
    try:
        ...
        logger.error("del_cat error: %s", e, exc_info=True)
        await call.answer("❌ خطایی رخ داد.", show_alert=True)
...
async def cb_admin_settings(call: CallbackQuery):
    try:
        ...
        logger.error("admin_settings error: %s", e, exc_info=True)
        await call.answer("❌ خطایی رخ داد.", show_alert=True)
...
async def _start_healthcheck(port: int):
    """Start a tiny aiohttp server for Railway's healthcheck.

    Railway assigns a `PORT` env var to every service (including Worker
    services). Binding to it lets Railway's TCP/HTTP probe see the
    process as alive even though the actual work is long-polling the
    Telegram Bot API.
    """
    try:
        ...
        logger.warning(
            "aiohttp is not available — healthcheck disabled. "
            "Install aiohttp>=3.9 in requirements.txt."
        )
        return None
    ...
    try:
        await site.start()
        logger.info("Healthcheck server listening on 0.0.0.0:%s", port)
    except OSError as e:
        logger.warning("Could not bind healthcheck to port %s: %s", port, e)
...
async def main() -> None:
    ...
    # Optional healthcheck so Railway sees the worker as healthy
    health_port = int(os.getenv("PORT", "8080"))
    health_runner = await _start_healthcheck(health_port)
